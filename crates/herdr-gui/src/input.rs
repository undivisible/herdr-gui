...
/// GPUI fires `observe_keystrokes` even when a `KeyBinding` handled the chord.
/// Swallow app shortcuts so the terminal does not also get the bare key (e.g. paste + trailing `v`).
